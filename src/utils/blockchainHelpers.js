...
const deployContractInner = (accounts, abi, deployOpts) => {
  ...
  return deploy.estimateGas({gas: MAX_GAS_PRICE}).then(estimatedGas => estimatedGas, err => console.log('errrrrrrrrrrrrrrrrr', err)).then(estimatedGas => {
...
let sendTX = (method, type) => {
  ...
  return new Promise((resolve, reject) => {
    method.on('error', error => {
      console.error(error)
      // https://github.com/poanetwork/ico-wizard/issues/472
      if (!error.message.includes('Failed to check for transaction receipt') && !error.message.includes('Failed to fetch') && !error.message.includes('Unexpected end of JSON input'))
        reject(error)
    })
...
const sendTXResponse = (receipt, type) => {
  if (0 !== + receipt.status || null === receipt.status) {
    return type === DEPLOY_CONTRACT
      ? Promise.resolve(receipt.contractAddress)
      : Promise.resolve()
  } else {
...
export function registerCrowdsaleAddress() {
  ...
  return Promise.all([whenRegistryAddress, whenAccount]).then(([registryAddress, account]) => {
    ...
    return registry.methods.add(crowdsaleAddress).send(opts)
...
export function loadRegistryAddresses() {
  ...
  return Promise.all([whenRegistryContract, whenAccount]).then(([registry, account]) => {
